...
def conf_delta_tensor(y_true, y_pred, anchors, ignore_thresh):
  ...
  anchors_ = tf.constant(anchors, dtype='float', shape=[1, 1, 1, 3, 2])  # exactly the same as anchor_grid

  true_wh = y_true[:, :, :, :, 2:4]
  true_wh = anchor_grid * tf.exp(true_wh)
  true_wh = true_wh * tf.expand_dims(y_true[:, :, :, :, 4], 4)  # reverse the operation _encode_box
...
def wh_scale_tensor(true_box_wh, anchors, image_size):
  ...
  wh_scale = tf.exp(true_box_wh) * anchors_ / image_size_  # _encode_box
...
def loss_coord_tensor(object_mask, pred_box, true_box, wh_scale, xywh_scale):
  ...
  # if has gt box then object_mask=1
...
def loss_conf_tensor(object_mask, pred_box_conf, true_box_conf, obj_scale, noobj_scale, conf_delta):
  ...
  criterion= keras.losses.binary_crossentropy
  loss_pos = object_mask_ * criterion(y_true=true_box_conf,y_pred=pred_box_conf,from_logits=True) * obj_scale
  loss_neg= (1 - object_mask_) * criterion(y_true=true_box_conf,y_pred=pred_box_conf,from_logits=True) * noobj_scale
  print(tf.reduce_sum(loss_pos))
  print(tf.reduce_sum(loss_neg))
...
def loss_class_tensor(object_mask, pred_box_class, true_box_class, class_scale):
  ...
  class_delta = object_mask * tf.expand_dims(
    tf.nn.sparse_softmax_cross_entropy_with_logits(labels=true_box_class_, logits=pred_box_class), 4) * class_scale
...
def _create_mesh_xy(batch_size, grid_h, grid_w, n_box):
  ...
  # Returns
      mesh_xy : Tensor, shape of (batch_size, grid_h, grid_w, n_box, 2)
          ...
          [..., 1] means "grid_h"
  """
  mesh_x = tf.cast(tf.reshape(tf.tile(tf.range(grid_w), [grid_h]), (1, grid_h, grid_w, 1, 1)), dtype=tf.float32)
