...
class BatchGenerator(object):
  ...
  def next_batch(self):
    ...
    return np.array(xs).astype(np.float32), \
           np.array(ys_1).astype(np.float32), \
           np.array(ys_2).astype(np.float32), \
           np.array(ys_3).astype(np.float32)
...
def _encode_box(ys, original_box, anchor_box, net_w, net_h):
  x1, y1, x2, y2 = original_box
  _, _, anchor_w, anchor_h = anchor_box

  # determine the yolo to be responsible for this bounding box
  grid_h, grid_w = ys.shape[:2]
