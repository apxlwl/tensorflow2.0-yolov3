...
def decode_netout(netout, anchors, obj_thresh, net_size,pad_scale, nb_box=3):
  ...
  netout = tf.reshape(netout,(n_rows, n_cols, nb_box, -1))
  boxes = []
...
def _activate_probs(objectness, classes, obj_thresh=0.3):
  ...
  objectness_prob = tf.sigmoid(objectness)
  classes_probs = tf.sigmoid(classes)
  # 2. conditional probability
  classes_conditional_probs = classes_probs * objectness_prob
  # 3. thresholding
  classes_conditional_probs *= tf.cast(objectness_prob > obj_thresh,tf.float32)
